    ...
    def update(self,passFail,partNum,failName):
        ...
        if (self.count < 5):
            ...
            if (self.val < 97 and self.count > 5 and WARNING_COUNT < 2):
                if (EMAIL_62 == 0 and self.lineName == "TS2000"):
                    WARNING_COUNT += 1
                    EMAIL_62 = 1
                    TSD_Email.send_email(self.lineName, self.val)
                elif (EMAIL_64 == 0 and self.lineName == "TS2000 B"):
                    WARNING_COUNT += 1
                    EMAIL_64 = 1
                    TSD_Email.send_email(self.lineName, self.val)
            ...
            if (failName != None):
                self.failList.insert("",0,text = time.asctime( time.localtime(time.time()) ), values = (failName,getFix(failName)))
                fo = open("Records\\TSD_Fail_Record.txt", "a")
                fo.write("%s %s %s %s %s\n" % (time.asctime( time.localtime(time.time())),self.lineName,self.currPart,failName,getFix(failName)))
                fo.close()
