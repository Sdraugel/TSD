...
class TSD_Parser:
    ...
    def __init__(self,machine1,machine2):
        ...
        self.priorityFile = priorityOpen()
        self.failList = None
##        old init
...
##        self.machine2 = TSD_Graphics.TSD_Graphics(self.win,machine2,record2,500,0)
    ...
    def checkTestPoints(self):
        ...
        book = xlrd.open_workbook(self.excelFile)
        first_sheet = book.sheet_by_index(0)
        book2 = xlrd.open_workbook(self.priorityFile)
        first_sheet2 = book2.sheet_by_index(0)

        p_size = first_sheet2.nrows
        counter = 0
                ...
                if (self.failList is not None):
                    ...
                    for char in self.failList:
                        ...
                        if char == "_":
                            ...
                            failure += char
                    for j in range (0, int(p_size), 1):
                        system = ""
                        system_value = str(first_sheet2.cell(j,0).value)
                        for char2 in system_value:
                            if char2 == "T":
                                system = system
                            if char2 == "S":
                                system = system
                            ...
                            if (str(system) == str(failure)):
                                print("Failure: " + str(failure) + "Recommended Fix: " + str(first_sheet2.cell(j, 3)))
                                returned_data = str(failure) + "," + str(first_sheet2.cell(j, 3).value)
                                self.failList = str(returned_data)
